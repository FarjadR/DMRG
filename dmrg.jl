...
create_file("energyF.txt",evod,mmax,Nsites)
create_file("energyFp.txt",evod,mmax,Nsites)
...
let
	...
	if Nstates != 0 
		...
		# PN addition: orthoginalize excited states; even if that are not converged, they form a basis to diagonalize H
		...
		for istates=1:Nstates+1
			...
			text_energyF*=string(F.values[istates]," ")
			text_energyFp*=string(Fp.values[istates]," ")
		...
		write_text("energyF.txt",g,text_energyF)
		write_text("energyFp.txt",g,text_energyFp)
